...
export default function LogProgress() {
  ...
  const displayDate = format(new Date(targetDate + "T12:00:00"), "MMMM d, yyyy");
  ...
  const onSave = async () => {
    ...
    try {
      ...
      // UPSERT: update if already logged for this date, insert if not
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.bg }]}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          <InputRow label="Body Weight" unit="lbs" value={weight} onChangeText={setWeight} colors={colors} />
          <InputRow label="Fat %" unit="%" value={fatPct} onChangeText={setFatPct} colors={colors} />
        </ScrollView>
      </KeyboardAvoidingView>
    </SafeAreaView>
  );
...
function InputRow({ label, unit, value, onChangeText, colors }: {
  label: string; unit: string; value: string; onChangeText: (v: string) => void; colors: any;
}) {
  return (
    <View style={[styles.inputRow, { borderBottomColor: colors.border }]}>
      <Text allowFontScaling={false} style={[styles.inputLabel, { color: colors.text }]}>{label}</Text>
      <View style={styles.inputRight}>
        <TextInput value={value} onChangeText={onChangeText} keyboardType="decimal-pad" placeholder="—"
          placeholderTextColor={colors.textMuted} style={[styles.input, { color: colors.text }]} allowFontScaling={false} />
        <Text allowFontScaling={false} style={[styles.inputUnit, { color: colors.textMuted }]}>{unit}</Text>
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  inputRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 16, borderBottomWidth: StyleSheet.hairlineWidth },
