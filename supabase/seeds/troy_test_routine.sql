-- ============================================================================
-- ONE-OFF SEED: Troy's "Upper/Lower + Arms" routine for his test client.
-- NOT a real migration — do NOT add to migration history. Paste this whole
-- file into Supabase Studio → SQL Editor and run once.
--
-- Before running:
--   1. Replace TEST_CLIENT_EMAIL on the line below with the actual email
--      of your test client (the user you've already invited & accepted as
--      a client of the trohoy123@gmail.com coach account).
--   2. Confirm trohoy123@gmail.com is your coach email; change if not.
--
-- The script is idempotent-ish: if you re-run, you'll create *another*
-- program. Delete the previous one first if you want a clean re-seed:
--   DELETE FROM coaching_programs WHERE name = 'Upper/Lower + Arms'
--     AND coach_id = (SELECT id FROM auth.users WHERE email = 'trohoy123@gmail.com');
-- ============================================================================
...
BEGIN
  ...
  IF v_client_id IS NULL THEN
    RAISE EXCEPTION 'Client not found: %  (edit CLIENT_EMAIL at the top of this file)', CLIENT_EMAIL;
  END IF;

  -- Make sure the coach-client relationship exists and is active. The
  -- mobile app filters program reads by client_id; the coach-client row
  -- gates RLS for everything cross-user.
  ...
  INSERT INTO coaching_programs (
    ...
    v_client_id,
    'Upper/Lower + Arms',
    '5-day hypertrophy split with chest+lats / quad / back-thickness / glutes+hams / shoulders+arms days. RIR 2 default.',
    'active',
    CURRENT_DATE
  )
  RETURNING id INTO v_program_id;

  -- Single phase (Block 1, 4 weeks, hypertrophy goal)
