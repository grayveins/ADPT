...
export const FULL_BODY_2X: SplitTemplate = {
  id: 'full_body_2x',
  name: 'Full Body 2x',
  daysPerWeek: 2,
...
export const FULL_BODY_3X: SplitTemplate = {
  id: 'full_body_3x',
  name: 'Full Body 3x',
  daysPerWeek: 3,
  suitableFor: ['beginner', 'intermediate'],
...
export const UPPER_LOWER_4X: SplitTemplate = {
  id: 'upper_lower_4x',
  name: 'Upper/Lower 4x',
  daysPerWeek: 4,
...
export const PPL_5X: SplitTemplate = {
  id: 'ppl_5x',
...
export const PPL_6X: SplitTemplate = {
  id: 'ppl_6x',
...
export const BRO_SPLIT_5X: SplitTemplate = {
  id: 'bro_split_5x',
  name: 'Bro Split 5x',
  daysPerWeek: 5,
  suitableFor: ['intermediate', 'advanced'],
 ...
 * 
 * Priority order for each day count:
 * - 2 days: Full Body 2x
 * - 3 days: Full Body 3x
 * - 4 days: Upper/Lower 4x
 * - 5 days: PPL 5x (Push/Pull/Legs + Upper/Lower)
 * - 6 days: PPL 6x (Push/Pull/Legs x2)
 */
export function getRecommendedSplit(
  daysPerWeek: number,
  experience: ExperienceLevel
): SplitTemplate {
  // Direct mapping by days - prioritize the most common/effective splits
  switch (daysPerWeek) {
    case 2:
      return FULL_BODY_2X;
    case 3:
    ...
    case 5:
      // PPL 5x is generally better for intermediate/advanced
      // Bro split is an alternative but not the default
      return PPL_5X;
    case 6:
      return PPL_6X;
    default:
      // Handle edge cases
      if (daysPerWeek < 2) return FULL_BODY_2X;
      if (daysPerWeek > 6) return PPL_6X;
      return FULL_BODY_3X;
  }
}
...
export function getSplitsForExperience(experience: ExperienceLevel): SplitTemplate[] {
  return ALL_SPLITS.filter(s => s.suitableFor.includes(experience));
}
...
export function getSplitById(id: string): SplitTemplate | undefined {
  return ALL_SPLITS.find(s => s.id === id);
}
 ...
 */
export function mapUserPreferenceToSplit(
  ...
  switch (preference) {
    case 'ppl':
    case 'push_pull_legs':
      return daysPerWeek >= 6 ? PPL_6X : PPL_5X;
    case 'upper_lower':
      return UPPER_LOWER_4X;
    case 'full_body':
      return daysPerWeek >= 3 ? FULL_BODY_3X : FULL_BODY_2X;
    case 'bro_split':
      return BRO_SPLIT_5X;
    case 'auto':
    default:
 ...
 */
export const SPLIT_DISPLAY_NAMES: Record<string, string> = {
